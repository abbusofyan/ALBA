...
<Layout>
	<v-breadcrumbs
	  ...
	  :items="school
	    ? [
	        { title: 'School', disabled: false, href: '/schools' },
	        { title: school.name, disabled: false, href: `/schools/${school.id}` },
	        { title: 'Edit', disabled: true }
	      ]
	    : [
	        { title: 'School', disabled: false, href: '/schools' },
	        { title: 'Create School', disabled: true }
	      ]"
	>
	  <template v-slot:prepend></template>
	</v-breadcrumbs>
    <VCard class="mb-5">
		<VForm @submit.prevent=" school && school.id ? form.put(route('schools.update', school.id)) : form.post(route('schools.store'))">
			<VCardItem>
	            ...
	            <VRow>
	                <VCol cols="6">
	                    <div class="flex items-center flex-none order-0 grow-0">
	                        <label class="v-label mb-1 text-body-2 text-wrap" for="v-67" style="line-height: 15px;">School Name <span class="ml-1" style="color: red">*</span></label>
					...
					<VCol cols="6">
	                    <div class="flex items-center flex-none order-0 grow-0">
	                        ...
	                        <AppTextField v-model="form.username" label="" placeholder="Username" readonly />
	                ...
	                <VCol cols="6">
	                    <div class="flex items-center flex-none order-0 grow-0">
	                        <label class="v-label mb-1 text-body-2 text-wrap" for="v-67" style="line-height: 15px;">Password <span class="ml-1" style="color: red">{{school ? '' : '*'}}</span></label>
					...
					<VCol cols="6">
					   <div class="flex items-center flex-none order-0 grow-0">
						   <label class="v-label mb-1 text-body-2 text-wrap" for="v-67" style="line-height: 15px;">Confirm Password <span class="ml-1" style="color: red">{{school ? '' : '*'}}</span></label>
	...
	<VCard v-if="school && $filters.can('delete-school')">
		<VCardItem>
			...
			<VBtn class="mt-5" type="submit" :disabled="!confirmDelete" color="error" @click="deleteSchool(school.id)">
				Delete Account
			</VBtn>
		</VCardItem>
    </VCard>
...
onMounted(() => {
    ...
    if (props.school) {
		...
		form.username = props.school.username;
		form.email = props.school.email;
		form.phone = props.school.phone;
		form.status = props.school.status
		form.address = props.school.address;
		form.postal_code = props.school.postal_code;
		form.can_order_pickup = props.school.can_order_pickup;
		form.secondary_email = props.school.secondary_email.map((email) => {
			return email.email
		})

		if (props.school.bins) {
		    props.school.bins.forEach(bin => {
		        form.bins.push({
		            id: bin.id,
